...
import { useCreateFieldMutation } from "@/hooks/queries/useFieldsMutations";
...
export default function CreateFieldPage() {
  ...
  const canCreateField = currentUserRole === "OWNER" || currentUserRole === "ADMIN";
  ...
  const onSubmit = async (data: CreateFieldFormData) => {
    ...
    if (!teamId) {
      setErrorMessage("Time inválido para criação do campo.");
      return;
    }
    ...
    try {
      ...
      setErrorMessage(
        getApiErrorMessage(error, "Não foi possível criar o campo."),
      );
    }
  };
    ...
    return (
      <AppShell>
        <div className="mx-auto max-w-4xl rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
          <p className="text-sm text-gray-600">Carregando informações do time...</p>
        </div>
      </AppShell>
    );
  }
    ...
    return (
      <AppShell>
        <div className="mx-auto max-w-3xl space-y-3 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-amber-900">
          <p>Apenas OWNER ou ADMIN do time podem criar campos.</p>
    ...
    <AppShell>
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="rounded-3xl border border-primary/20 bg-white p-6 shadow-sm">
          <h1 className="text-2xl font-bold text-primary">Criar campo do time</h1>
          <p className="mt-1 text-sm text-gray-600">
            Cadastre um novo campo para <span className="font-semibold">{team.name}</span>.
          </p>
        </div>
                ...
                {uploadedPhotos.map((photo) => (
                  ...
                  >
                    ...
                    <Button
                      ...
                      disabled={isSubmitting || createFieldMutation.isPending}
                    >
                      Remover
                    </Button>
                  </div>
